    ...
    def clean(self, value):
        ...
        if isinstance(value, dict):
            ...
            if link_type == "internal" and not internal_page:
                errors["internal_page"] = ValidationError(
                    'Bitte wählen Sie eine Seite aus, wenn Sie "Interne Seite" als Link-Typ ausgewählt haben.'
                )
            ...
            if link_type == "external" and not external_url:
                errors["external_url"] = ValidationError(
                    'Bitte geben Sie eine URL ein, wenn Sie "Externe URL" als Link-Typ ausgewählt haben.'
                )

            # If link_type is None and a link is provided
            if link_type is None and (internal_page or external_url):
                errors["link_type"] = ValidationError(
                    "Bitte wählen Sie einen Link-Typ aus, wenn Sie einen Link angeben."
                )
...
def create_multi_column_block(content_blocks=None):
    ...
    class MultiColumnBlock(StructBlock):
        max_columns = ChoiceBlock(
            choices=[("1", "1 Spalte"), ("2", "2 Spalten"), ("4", "4 Spalten")],
            default="1",
